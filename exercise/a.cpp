...
int main() {
    ...
    while (q--) {
        ...
        if (l == r) {
            ...
            result = get_left_dis(r, ridx) + get_right_dis(l, lidx) + get_right_dis(r, ridx) -
                     get_right_dis(mid, midx) + get_left_dis(l, lidx) -
                     get_left_dis(mid, midx) - (ridx - midx) * (r - mid) - (midx - lidx) * (mid - l);
            printf("%lld\n", result);
